...
if(!empty($_POST['recherche'])){
    ...
    $req1 = $pdo->query('SELECT membres.tel, services.titre_service, services.photo_service, services.prix_service FROM membres INNER JOIN services ON membres.id = services.id_artisan WHERE titre_service REGEXP "' .$recherche .'"');
    ...
    <div class="services-prev">
      <div class="services">
        <?php if(!empty($services)): ?>
          ...
          <?php endforeach ?>
        <?php else:?>
            <div>Très cher client, aucun service ne correspond à votre recherche</div>
        <?php endif ;?>
      </div>
    </div>
